...
class LogManagement():
                    ...
                    if len(detected_response['hits']['hits']) > 0:
                        ...
                        for detect_hit in detected_response['hits']['hits']:
                            ...
                            if self.is_rule_match(detected_log, log):  # 매칭 조건 확인
                                ...
                                rule_info = self.es.search(index=f"{self.system}_ruleset", body={
                                                           "query": {"bool": {"must": [{"match": {"name": detected_by_rules}}]}}})
                                severities.append(
                                    rule_info['hits']['hits'][0]['_source']['severity'])
    ...
    def fetch_log_properties(self):
        ...
        properties_list = [{key: list(values)}
